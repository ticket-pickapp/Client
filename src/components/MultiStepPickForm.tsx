...
const MERCADOS_PRINCIPALES = [
  "1X2",
  "Over/Under",
  "Doble Oportunidad",
];
const MERCADOS_AVANZADOS = [
  "Corners",
  "Tarjetas",
  "Hándicaps",
  "BTTS",
...
export default function MultiStepPickForm({ onCancel, onSuccess }: { onCancel?: () => void; onSuccess?: () => void }) {
  ...
  const [openDropdown, setOpenDropdown] = useState<'principal' | 'avanzado' | 'especial' | null>('principal');
  ...
  const handleCheckbox = (tipo: string, valor: string) => {
    setForm({ ...form, mercado: valor, tipoMercado: tipo as 'principal' | 'avanzado' | 'especial' });
    // Ya no cerramos el dropdown automáticamente
  };

  // Paso 3: Análisis y monetización
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>, tipo: "audio" | "imagen") => {
  ...
  const paso3Valido = form.analisis.length > 0 || audioFile || imageFile || form.plan || form.gratuito;

  // Navegación
  const siguiente = () => setStep((s) => Math.min(s + 1, 4));
  const atras = () => setStep((s) => Math.max(s - 1, 1));
  const omitir = () => setStep(4);
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-4 mb-6">
            ...
            <div className="border rounded-lg">
              ...
              >
                Mercado principal
                {openDropdown === 'principal' ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
              </button>
            ...
            <div className="border rounded-lg">
              <button
                ...
                onClick={() => setOpenDropdown(openDropdown === 'avanzado' ? null : 'avanzado')}
                disabled={!!form.mercado && form.tipoMercado !== 'avanzado'}
              >
                Mercado avanzado
                {openDropdown === 'avanzado' ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
              </button>
                ...
                <div className="px-4 pb-3 pt-1 space-y-2">
                  {MERCADOS_AVANZADOS.map((m) => (
                    <button
                      key={m}
                      type="button"
                      className={`w-full text-left px-3 py-2 rounded-md font-medium transition-colors ${form.mercado === m && form.tipoMercado === 'avanzado' ? 'bg-[#1e9df1] text-white' : 'bg-[#E3ECF6] text-[#0f1419] hover:bg-[#d0eafd]'}`}
                      onClick={() => { handleCheckbox('avanzado', m); }}
                    >
                      {m}
                    </button>
                  ))}
            ...
            <div className="border rounded-lg">
              <button
                type="button"
                className="w-full flex items-center justify-between px-4 py-3 text-left text-[#0f1419] font-semibold focus:outline-none"
                onClick={() => setOpenDropdown(openDropdown === 'especial' ? null : 'especial')}
                disabled={!!form.mercado && form.tipoMercado !== 'especial'}
              >
                Especiales
                {openDropdown === 'especial' ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
              </button>
                ...
                <div className="px-4 pb-3 pt-1 space-y-2">
                  {MERCADOS_ESPECIALES.map((m) => (
                    <button
                      key={m}
                      type="button"
                      className={`w-full text-left px-3 py-2 rounded-md font-medium transition-colors ${form.mercado === m && form.tipoMercado === 'especial' ? 'bg-[#1e9df1] text-white' : 'bg-[#E3ECF6] text-[#0f1419] hover:bg-[#d0eafd]'}`}
                      onClick={() => { handleCheckbox('especial', m); }}
                    >
                      {m}
                    </button>
                  ))}
      ...
      {step === 3 && (
        <div>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-4">
            <div className="flex-1">
              <label className="block text-[#0f1419] mb-1">Subir audio (opcional)</label>
              <input type="file" accept="audio/*" onChange={(e) => handleFile(e, "audio")}/>
            </div>
            <div className="flex-1">
              <label className="block text-[#0f1419] mb-1">Subir imagen (opcional)</label>
              <input type="file" accept="image/*" onChange={(e) => handleFile(e, "imagen")}/>
            </div>
          </div>
